...
class Withdraw_model extends MY_Model {
    ...
    //拒绝提现，解除冻结都在这里实现
    public function refuseWithdraw($withdraw){

        $result = FALSE;

        $this->db->trans_start();

        $coin = $this->coin_model->one($withdraw['withdraw_coin']);

        $oldWithdraw = $this->one($withdraw['withdraw_id']);

        $userCoinAsset = $this->asset_model->oneAssetByUserAndCoinAndPlate($withdraw['withdraw_user'], $withdraw['withdraw_coin'], 1);

        if ($coin && $oldWithdraw && $oldWithdraw['withdraw_status'] != 2 && $userCoinAsset) {

            if (bccomp($userCoinAsset['asset_frozen'], $withdraw['withdraw_amount']) >= 0) {
                
                //不通过，解除冻结
                $userCoinAsset['asset_frozen'] = bcsub($userCoinAsset['asset_frozen'], $withdraw['withdraw_amount']);

                if ($this->asset_model->update($userCoinAsset)) {
                    
                    //不通过再加款
                    $assetResult = $this->asset_model->assetChange($withdraw['withdraw_user'], $coin['coin_symbol'], $withdraw['withdraw_amount'], 3);
    ...
    public function successWithdraw($withdraw){
        ...
        if ($coin && $oldWithdraw && $oldWithdraw['withdraw_status'] != 6 && $userCoinAsset) {
            ...
            if ($this->asset_model->update($userCoinAsset)) {
                ...
                //写入资产记录
                $result = $this->asset_log_model->insertWithdraw($withdraw);
