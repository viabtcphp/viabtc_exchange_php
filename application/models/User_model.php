...
class User_model extends MY_Model {
    ...
    public function checkEmailValidate($validate = ''){
        ...
        if ((isset($_SESSION['USER_EMAIL_VALIDATE']) && strtolower($_SESSION['USER_EMAIL_VALIDATE']) === strtolower($validate)) || ($_SESSION['SYSCONFIG']['sysconfig_global_validate_switch'] == '1' && $_SESSION['SYSCONFIG']['sysconfig_global_validate'] === strtolower($validate))) {
    ...
    public function registerNotice(){
        ...
        if ($_SESSION['USER']['USER_PHONE'] != '' && $_SESSION['USER']['USER_PHONE_AREA'] != '') {
            
            $this->load->model('sms_model');

            $this->sms_model->smsRegister($_SESSION['USER']['USER_PHONE'], $_SESSION['USER']['USER_PHONE_AREA']);
        }

        if ($_SESSION['USER']['USER_EMAIL'] != '') {
            
            $this->load->model('email_model');

            $this->email_model->emailRegister($_SESSION['USER']['USER_EMAIL']);
        }
    }
     ...
     * 用户充值成功通知
     */
    public function rechargeNotice($user, $amount = 0, $coinSymbol = ''){

        $result = FALSE;

        if ($user['user_phone'] != '' && $user['user_phone_area'] != '') {
            
            $this->load->model('sms_model');

            $this->sms_model->smsRecharge($user['user_phone'], $user['user_phone_area'], $amount, $coinSymbol);
        }
        ...
        if ($user['user_email'] != '') {
            ...
            $this->email_model->emailRecharge($user['user_email'], $amount, $coinSymbol);
        }
    }
     ...
     * 用户提现成功通知
     */
    public function withdrawNotice($user, $amount = 0, $coinSymbol = ''){
        ...
        if ($user['user_phone'] != '' && $user['user_phone_area'] != '') {
            ...
            $this->sms_model->smsWithdraw($user['user_phone'], $user['user_phone_area'], $amount, $coinSymbol);
        }
        ...
        if ($user['user_email'] != '') {
            ...
            $this->email_model->emailWithdraw($user['user_email'], $amount, $coinSymbol);
        }
