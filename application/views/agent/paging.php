...
<div class="paging layui-form">
    <div class="layui-inline">
        ...
        <?php }else{?>
            <a class="layui-btn layui-btn-sm" href="<?php echo $pagingInfo['action']; ?>1.html">首页</a>
            <a class="layui-btn layui-btn-sm" href="<?php echo $pagingInfo['action'] . ($pagingInfo['pageindex'] - 1); ?>.html">上一页</a>
        <?php }?>
        ...
        <?php for($i=$pagingInfo['pageindexstart'];$i<=$pagingInfo['pageindexend'];$i++){
            if($i == $pagingInfo['pageindex'])
                ...
                echo "<a class='layui-btn layui-btn-sm' href='" . $pagingInfo['action'] . $i . ".html'>" . $i . "</a>";
        }?>
        ...
        <?php }else{?>
            <a class="layui-btn layui-btn-sm" href="<?php echo $pagingInfo['action'] . ($pagingInfo['pageindex'] + 1); ?>.html">下一页</a>
            <a class="layui-btn layui-btn-sm" href="<?php echo $pagingInfo['action'] . $pagingInfo['pagesum']; ?>.html">末页</a>
        <?php }?><br />
    </div>
    <div class="layui-inline">
        <select lay-filter="paging" url="<?php echo $pagingInfo['action']; ?>" class="page_select">
            <?php for($i = 1; $i <= $pagingInfo['pagesum']; $i ++){ ?>
            <option value="<?php echo $i; ?>.html" <?php echo ($i==$pagingInfo['pageindex']) ? "selected='selected'" : '' ?>><?php echo $i; ?></option>
            <?php } ?>
        </select>
    </div>
</div>
