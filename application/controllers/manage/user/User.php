	...
	public function user_dm($userId = 0, $pageIndex = 1){
		...
		if ($userId > 0) {
			...
			if ($user && is_array($user) && count($user)) {
				...
				$pagingInfo = getPagingInfo($dmCount, $pageIndex, $this->pageSize, $this->config->item('manage_page'), base_url('/manage/user/user/user_dm/' . $userId . '/'));
				...
				$this->load->view('manage/user/user_dm', $data);
			}
		}
	}
}