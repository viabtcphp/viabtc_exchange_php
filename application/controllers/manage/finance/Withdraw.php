		...
		if ($_POST) {
			...
			if ($withdraw) {
				...
				if (intval($this->input->post('action'))) {
					...
					$udunResult = $this->udun_model->withdraw($withdraw['withdraw_chain'], $withdraw['withdraw_contract'], $withdraw['withdraw_finally_amount'], $withdraw['withdraw_to_address'], $withdraw['withdraw_no'], $withdraw['withdraw_to_address_memo']);

					if ($udunResult) {

						if ($withdraw['withdraw_local'] == 1) {
							
							unset($withdraw['withdraw_status']);
						}else{

							$withdraw['withdraw_status'] = 3;
						}

						if ($this->withdraw_model->update($withdraw)) {
							
							$result['status'] = TRUE;
							$result['message'] = '已转发到钱包';
						}
					}else{

						$result['message'] = '转发到钱包失败，请检查钱包';
					}
