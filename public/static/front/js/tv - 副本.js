...
var tvCurrentData = {
  ...
  coinSymbol: 'BTC', // 交易币种
  baseScale: 2, // USDT精度 保留2位小数
  coinScale: 2, // BTC精度 保留2位小数
  volumeScale: 2, // 成交量精度保留 2位小数
  interval: '15', // 15分钟
  chartType: 1, // 默认蜡烛图：1， 分时图：3 int类型
  bgColor: tvBlackBgColor // 当前选择背景色
}
...
function datafeedCreate() {
  return {
    ...
    getBars: function (symbolInfo, resolution, from, to, onHistoryCallback, onErrorCallback, firstDataRequest) {
      ...
      tvHistoryCallbackMap[key] = function (data) {
        ...
        if (data.result && data.result.length) {
          ...
          isD_W_M && console.log(interval, isD_W_M)
    ...
    subscribeBars: function (symbolInfo, resolution, onRealtimeCallback, subscribeUID, onResetCacheNeededCallback) {
      ...
      socket.send(JSON.stringify(lastData)) // 取消订阅
      ...
      socket.send(JSON.stringify(senData)) // 新订阅
    },
...
// init函数
function init(cb) {
  socket = new WebSocket('ws://103.43.160.147:8099')
    ...
    if (tvHistoryCallbackMap[key]) {
      ...
      if (typeof realtimeCallback === 'function') {
        ...
        if (symbol === requestSymbol()) {
          ...
          var interval = tvCurrentData.interval
          var isD_W_M = (interval === 'D') || (interval === 'W') || (interval === 'M') // 周期是DWM就加8小时
          isD_W_M && console.log('isD_W_M', isD_W_M)
          if (isD_W_M) {
            time = (item[0] + 8 * 60 * 60) * 1000
          }
          var barNew = {
